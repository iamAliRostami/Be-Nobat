...
Android `Application` entry point. It exposes logging tags, starts Koin with all modules, loads the
saved theme from `ThemeManager`, and applies it before normal UI usage. Locale initialization belongs
to `StartActivity`, after AppCompat has restored persisted application locales.

| Function | Responsibility |
|---|---|
| `onCreate()` | Starts Koin, registers Android context/modules, reads the persisted theme, and applies night mode. |
...
| `StartActivity.applyDefaultLanguageIfNeeded()` | Selects Persian only when AppCompat has no restored application locale and stops the old Activity instance from routing. |
...
نقطه شروع Application اندروید است؛ Tagهای لاگ را ارائه می‌کند، Koin را راه‌اندازی می‌کند و تم
ذخیره‌شده را بارگذاری می‌کند. تنظیم locale پس از بازیابی زبان ذخیره‌شده برعهده `StartActivity` است.
...
| `StartActivity.applyDefaultLanguageIfNeeded()` | فقط در نبود locale بازیابی‌شده فارسی را انتخاب می‌کند و مانع navigation نمونه قدیمی Activity می‌شود. |
...
| Startup locale initialization | Moved first-launch locale initialization from `App` to `StartActivity` so AppCompat can restore saved locales first. | تنظیم زبان اولین اجرا از `App` به `StartActivity` منتقل شد تا ابتدا locale ذخیره‌شده بازیابی شود. |
